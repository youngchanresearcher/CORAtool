...
> 絕對秒數會隨機器浮動——同一個容器稍早跑同一支腳本得到 0.16 / 0.85 / 4.0 / 23 秒——所以請看成長倍率，不要看秒數本身。Python 版也是一樣的指數成長（18 層級 5.9 秒，另一次量測），這是演算法本身的性質，不是移植造成的。
>
> 層級數超過 30 的條件在 `"ON-DC"` 下會直接報錯（那是化簡步驟所用位元遮罩的寬度），錯誤訊息會指名是哪個條件並建議改用 `"ON-OFF"`；`"ON-OFF"` 沒有這個限制，40 個層級和 4 個層級一樣快。
...
- 但 `aut` 在 CRAN 的慣例裡意味著**對這個套件的內容負責**。本套件在七個地方刻意偏離了 Python 版（§8、附錄 A），原作者沒有參與這些判斷，也沒有機會審閱。把他們列為 `aut` 等於讓他們為自己不同意、甚至不知情的改動背書。
- 實務上還有一層：CRAN 審查有時會要求說明 `aut` 的貢獻內容。「他們寫了另一個語言的原版，我改了七個地方」不是 `aut` 的定義。
...
README 開頭與附錄 A.8 都寫了這句話。這不只是客套——附錄 A 的六條每一條都附了原始碼位置和可重現的例子，就是為了讓這句話可以被檢驗。
