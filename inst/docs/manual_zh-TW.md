...
「從 0 開始」不是形式要求，是實質要求。一個編碼成 `{1, 2}` 而非 `{0, 1}` 的條件會讓 `"ON-OFF"` 演算法算出錯誤結果（見 §7.4）。套件偵測到這種編碼時會發出警告。
...
**實務建議：預設的 `"ON-DC"` 是安全選擇。** 在 10 組二元配對資料上，兩種演算法產生的質蘊涵項與解完全一致。但**條件沒有從 0 開始編碼時，`"ON-OFF"` 會算出錯誤的覆蓋集與分數**（見 §7.4），`"ON-DC"` 不受影響。編碼正確時，條件數多的資料用 `"ON-OFF"` 會快很多。
...
| **條件沒從 0 編碼** | 見 §7.4。這個錯誤不會報錯，只會給出看起來合理的錯數字。 |

### 7.4 條件必須從 0 開始編碼

這是唯一一個會**安靜地產生錯誤數字**的陷阱，值得單獨說明。

`"ON-OFF"` 演算法在把化簡結果還原成完整的條件組合時，對「不在意」的條件填入 `{0, 1, ..., levels-1}`——也就是**假設值從 0 開始**。當某個條件實際編碼成 `{1, 2}` 時，這個集合變成 `{0, 1}`，於是所有該條件為 2 的案例都被排除在覆蓋之外。

以 Python 版 README 自己的範例為例（條件 B 編碼為 `{1, 2}`）：

| 質蘊涵項 | ON-DC 覆蓋 | ON-DC Cov. | ON-OFF 覆蓋 | ON-OFF Cov. |
|---|---|---|---|---|
...
本套件忠實複製了這個行為（移植的目的是重現參考實作），但**新增了一個警告**：
...
**處理方式**：把條件重新編碼成從 0 開始（`B - 1`），或改用 `"ON-DC"`。`"ON-DC"` 的化簡是直接在真值表的實際值上進行的，不受影響。
