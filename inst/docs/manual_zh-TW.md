...
- [附錄 A：Python 版的五個缺陷](#附錄-apython-版的五個缺陷)——每一條都附原始碼位置與可重現的例子
...
| **拿兩次輸出做字串比對** | 連言內部的字面**依資料的欄位順序印出**。同一份資料把欄位換個順序，`A{0}*C{1}` 會變成 `C{1}*A{0}`——**同一個質蘊涵項、同一組解**，只是字面的排列不同（Python 版也是如此）。要比對兩次分析的結果，比對字面的集合，不要比對字串。 |
...
這五處都是本套件自己的判斷，不是原作者的。**每一處的原始碼位置、可重現的例子與完整推導見附錄 A**；與 QCA、QCApro、cna 的設計比較見附錄 B。
...
五個缺陷裡有三個源自同一個假設。Python 版計算每個條件的**層級數**如下：
...
上面五條都是「不同之處」。每一條都附了原始碼位置與可重現的例子，就是為了讓你能自己判斷我改得對不對，而不是要你相信我。
