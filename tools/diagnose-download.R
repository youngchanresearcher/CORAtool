...
if (!file.exists(f)) {
  ...
  say('  "C:/Users/you/Downloads/CORA_0.1.1.tar.gz"')
} else {
  info <- file.info(f)
  ...
  if (grepl(" ", basename(f))) {
    ...
    say("      CORA_0.1.1.tar.gz, with an underscore. A renamed file usually")
  ...
  if (length(magic) >= 3L &&
    ...
    if (inherits(listing, "try-error")) {
      ...
      say("entries      : ", length(listing), "  (a complete CORA has 78)")
  ...
  if (grepl("OneDrive", f, ignore.case = TRUE)) {
    ...
    say('  file.copy(f, "C:/temp/CORA_0.1.1.tar.gz", overwrite = TRUE)')
    say('  install.packages("C:/temp/CORA_0.1.1.tar.gz",')
    say('                   repos = NULL, type = "source")')
  }
}
line()
